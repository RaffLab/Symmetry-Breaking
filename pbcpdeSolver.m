...
x = xlist(:);
x = [x;2*x(end) - x(end-1)]; % Add additional point to make periodic
ic = [ic , ic(:,1)];
...
options = odeset('NonNegative',1:length(ic2),'RelTol',1e-8,'AbsTol',1e-8);

s  = ode15s(@pdeodes,t',ic2,options);
sz = size(s.y);
sol=zeros([length(t),n,npde]);
for i=1:npde
    for j=1:n
        k=(i-1)*n+j;
